...
var intersections_on: bool = false
...
func check_for_door_combo(last3: Array[CardModel]) -> CardManager.CARD_COLOR:
	var crossoroads_count: int = 0
	var colors: Dictionary[CardManager.CARD_COLOR, bool] = {}
	#count the different colors
	for card_model in last3:
		#count how many crosroads cards are present
		if card_model.color == CardManager.CARD_COLOR.MULTI:
			crossoroads_count = crossoroads_count + 1
			continue
		colors[card_model.color] = true
		
	#check the colors number and the types
	if crossoroads_count > 1:
		# strict rules interpretation: don't allow combos with multiple crossoroads
		return CardManager.CARD_COLOR.NONE
	if intersections_on and (last3[0].color == CardManager.CARD_COLOR.MULTI or last3[2].color == CardManager.CARD_COLOR.MULTI):
		#crossroads is valid only if played as the middle card
		return CardManager.CARD_COLOR.NONE
	if colors.keys().size() == 1:
...
func get_save_dict() -> Dictionary[String, Variant]:
	var dict: Dictionary[String, Variant] = {
		...
		"intersections_on": intersections_on
	}
...
func load_game() -> void:
	...
	if error == OK:
		...
		if dict.has("deck"):
			...
			intersections_on = dict["intersections_on"]
