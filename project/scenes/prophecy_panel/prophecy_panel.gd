...
func _ready() -> void:
	SignalManager.swap_panel_cards.connect(swap_panel_cards)
	SignalManager.panel_cards_reset_position.connect(panel_cards_reset_position)
...
func set_panel_enabled(enabled: bool) -> void:
	...
	if visible:
		...
		for card_model in card_models:
			...
			var panel_card: PanelCard = create_panel_card(card_model, card_container, marker.position, PROPHECY_SIZE, true, 0)
...
func create_panel_card(model: CardModel, parent: Node2D, pposition: Vector2, pscale: Vector2, pickable: bool, pz_index: int) -> PanelCard:
...
func swap_panel_cards(card1: PanelCard, card2: PanelCard) -> void:
...
func panel_cards_reset_position(card: PanelCard) -> void:
